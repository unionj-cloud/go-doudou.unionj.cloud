...
- 如果Go版本低于1.17
```shell
go get -v github.com/unionj-cloud/go-doudou@v1.1.0
```

- 如果Go版本 >= 1.17，推荐采用如下命令全局安装`go-doudou`命令行工具
```shell
go install -v github.com/unionj-cloud/go-doudou@v1.1.0
```
推荐采用如下命令下载go-doudou作为项目的依赖
```shell
go get -v -d github.com/unionj-cloud/go-doudou@v1.1.0
```
